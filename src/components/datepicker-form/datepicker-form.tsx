...
export const DatePickerForm = ({ label, name } : { label?: string, name: string }) => {
    const [form] = Form.useForm();
    const { RangePicker } = DatePicker;
    const [values, setValues] = useState<[string, string] | undefined[]>([undefined, undefined]);

    type RangeValue = Parameters<NonNullable<React.ComponentProps<typeof DatePicker.RangePicker>['onChange']>>[0]

    function onSelectDate(dates: RangeValue | null, dateStrings: [string, string]) {
        console.log(dates);
    ...
    const handleSubmit = (values: [string, string] | undefined) => {
        if (values && (values[0] !== undefined && values[1])) setDates(values[0], values[1]);
    };
